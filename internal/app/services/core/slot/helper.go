...
func classifyDayCoverageFromSlots(slots []fhir_dto.Slot, expected []interval) (dayCoverage, []string) {
	...
	allFree, allAuto, nonAutoIDs, autoIDs := categorizeSlots(slots)
	if !allFree {
	...
	for _, s := range slots {
		...
		if auto {
			...
			if s.Status == fhir_dto.SlotStatusFree && s.ID != "" {
				nonAutoIDs = append(nonAutoIDs, s.ID)
			}
		}
	}
	return
...
func subtractBlocksFromInterval(iv interval, merged []conflictBlock) []interval {
	...
	for _, b := range merged {
		...
		gapBeforeBlock(&gaps, cur, b, iv.End)
		if b.End.After(cur) {
			cur = b.End
		}
...
// gapBeforeBlock appends a gap before a block if there's room.
func gapBeforeBlock(gaps *[]interval, cur time.Time, b conflictBlock, ivEnd time.Time) {
...
func BuildSlotAdjustmentForAppointment(
	...
	toDelete = freeSlotIDsNotInAdjusted(existingFree, adjusted)
	...
	toCreate = append(toCreate, busySlots...)

	return toDelete, toCreate, nil
}

// intersectIntervalsWithWindows returns appointment ∩ union(baseWindows)
// freeSlotIDsNotInAdjusted returns IDs of free slots whose interval is not in the adjusted set.
func freeSlotIDsNotInAdjusted(existingFree []fhir_dto.Slot, adjusted []interval) []string {
	set := make(map[string]struct{}, len(adjusted))
	for _, iv := range adjusted {
		set[intervalKey(iv.Start, iv.End)] = struct{}{}
	}
	var ids []string
	for _, slt := range existingFree {
		...
		if _, ok := set[intervalKey(slt.Start, slt.End)]; !ok {
